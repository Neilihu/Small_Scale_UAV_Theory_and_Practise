...
class intendedPath():
	def __init__(self, mode = 0,oX = 0, oY = 0,oZ = -VPC.InitialDownPosition, Course=0,R=50,Direction = 1):
		self.mode = mode
		self.oX = oX
		self.oY = oY
		self.oZ = oZ
	...
	def __str__(self):
		return "referenceCommands:(mode={}, origin={}, " \
			   "course={}, Radius={}, Direction={}".format(self.mode,(self.oX,self.oY,self.oZ),math.degrees(self.Course),self.R,self.Direction)

	def __eq__(self, other):
		if isinstance(other, type(self)):
			if not all([math.isclose(getattr(self, member), getattr(other, member), abs_tol=testingAbs_tol) for member in ['mode',
																								   'oX',
																								   'oY',
																								   'oZ',
																								   'Course',
